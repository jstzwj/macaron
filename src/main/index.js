...
// Make Macaron a single instance application.
if (!process.mas && process.env.NODE_ENV !== 'development') {
  const gotSingleInstanceLock = app.requestSingleInstanceLock()
  if (!gotSingleInstanceLock) {
    process.stdout.write('Other Macaron instance detected: exiting...\n')
    app.exit()
  }
}

// Macaron environment is configured successfully. You can now access paths, use the logger etc.
...
try {
  ...
  const msgHint = err.message.includes('Config schema violation')
    ...
    : ''
  log.error(`Loading Macaron failed during initialization! ${msgHint}`, err)
