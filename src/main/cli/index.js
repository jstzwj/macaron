...
const cli = () => {
  ...
  if (process.env.NODE_ENV === 'development') {
    // Don't pass electron development arguments to Macaron and change user data path.
    argv = ['--user-data-dir', path.join(getPath('appData'), 'macaron-dev')]
  }

  const args = parseArgs(argv, true)
  if (args['--help']) {
    write(`Usage: macaron [commands] [path ...]
...
`)
  ...
  if (args['--version']) {
    writeLine(`Macaron: ${global.MARKTEXT_VERSION_STRING}`)
  ...
  if (!args['--user-data-dir']) {
    const portablePath = path.join(app.getAppPath(), '..', '..', 'macaron-user-data')
